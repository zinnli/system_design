...
import { validateFiles } from "./validateFile";
import { withRetry } from "./withRetry";
import { DEFAULT_CHUNK_CONFIG, DEFAULT_VALIDATION_CONFIG } from "./config";
import type {
  ChunkUploadConfig,
  RejectedFile,
  UploadFileState,
 ...
 * React는 subscribe/getSnapshot을 통해 반환 객체를 구독만 한다 — 그래서
...
export function createUploadManager(
  ...
  function getSnapshot(): UploadFileState[] {
    if (!cachedSnapshot) {
      cachedSnapshot = Array.from(files.values());
    }
  ...
  function addFiles(incoming: File[]): { rejected: RejectedFile[] } {
    const existingNames = new Set(
      Array.from(files.values()).map((f) => f.file.name),
    );
    ...
    for (const file of accepted) {
      ...
      files.set(id, createInitialState(id, file));
      scheduleUpload(id);
    }
  ...
  function cancel(id: string): void {
    ...
    sessions.get(id)?.controller.abort(USER_CANCEL_REASON);
  ...
  function createInitialState(id: string, file: File): UploadFileState {
    return {
      id,
      file,
      status: "queued",
      progress: 0,
      uploadedBytes: 0,
      totalChunks: Math.ceil(file.size / chunkConfig.chunkSizeBytes),
      completedChunks: 0,
      error: null,
      attempt: 0,
    };
  ...
  function scheduleUpload(id: string): void {
    queueMicrotask(() => void startUpload(id));
  }
  ...
  async function startUpload(id: string): Promise<void> {
    ...
    const session = getOrCreateSession(id, state.file);
    const { signal } = session.controller;

    patch(id, { status: "uploading", error: null });

    try {
      const uploadId = await ensureUploadId(session, state.file, signal);

      await uploadRemainingChunks(id, session, uploadId);
      // in-flight 요청 없이 취소된 경우(모든 청크 태스크가 조용히 건너뜀)는
      // 예외가 발생하지 않으므로 여기서 한 번 더 확인한다.
      if (isUserCanceled(session)) return;

      await callWithRetry(() => completeUpload(uploadId, signal), signal);
      ...
      if (isUserCanceled(session)) return;
      patch(id, { status: "error", error: toErrorMessage(err) });
    }
  }

  function getOrCreateSession(id: string, file: File): UploadSession {
    ...
    const created: UploadSession = {
      ...
      chunks: chunkFile(file, chunkConfig.chunkSizeBytes),
  ...
  async function ensureUploadId(
    ...
    const { uploadId } = await callWithRetry(
    ...
    await Promise.all(
      remaining.map((index) =>
        pool.run(async () => {
          ...
          reportChunkProgress(id, session);
        }),
      ),
    );
  }
  ...
  function reportChunkProgress(id: string, session: UploadSession): void {
  ...
  function callWithRetry<T>(
    ...
    return withRetry(
      fn,
      chunkConfig.maxRetriesPerChunk,
      chunkConfig.retryBaseDelayMs,
      signal,
    );
  }

  return { subscribe, getSnapshot, addFiles, cancel, retry, remove };
}
